...
export function CampaignDialog({
  ...
  useEffect(() => {
    if (open) {
      ...
      setFromAlias("");
      setFilters({ query: "", company: "", position: "", tagIds: [] });
    }
  }, [open, templates]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (filters.position?.trim())
      cleanFilters.position = filters.position.trim();
  ...
  const canSubmit = name.trim() && templateId && !saving;
      ...
      <DialogContent className="border-slate-800 bg-slate-950 sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription className="text-slate-400">
            Creá una campaña seleccionando una plantilla y definiendo la
            audiencia
          </DialogDescription>
        </DialogHeader>
